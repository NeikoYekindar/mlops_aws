...
def main(args):
    # Load test data
    print(f"[TEST 3] Loading test data from: {args.data_path}")
    df = read_csv(args.data_path)
    
    # Evaluate both models (each with its own metadata-driven preprocessing)
    print(f"[TEST 3] Evaluating Model 1: {args.model1_path}")
    mse1, latency1 = evaluate_model(args.model1_path, df)
    
    print(f"[TEST 3] Evaluating Model 2: {args.model2_path}")
    ...
    print(f"[TEST 3] Best model for this test: {best_path}")
    ...
    results_file = os.path.join(out_dir, "results_test_3.json")
    with open(results_file, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print(f"[TEST 3] Results saved to: {results_file}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test 3: Compare two weather models on weather_test_3.csv")
    parser.add_argument('--data-path', type=str, required=True, help='Path to weather_test_3.csv')
